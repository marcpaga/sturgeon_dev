...
if __name__ == '__main__':
    ...
    parser.add_argument("--model-type", type=str, choices = ['ce', 'triplet'], default='ce')
    ...
    logger.info('Creating fake split')
    data_fake_labels = np.load(args.data_file)['diagnostics']
    split_df = pd.DataFrame({
        "model_n": [0] * len(data_fake_labels),
        "set": ['test'] * len(data_fake_labels),
        "sample": list(range(len(data_fake_labels))),
        "class": data_fake_labels,
    }).to_csv(os.path.join(args.output_dir, 'maxima_fake_split_tmp.csv'), header = True, index = False)
    ...
    dataset = MicroarrayDatasetFromOnlineSims(
        ...
        split_csv = os.path.join(args.output_dir, 'maxima_fake_split_tmp.csv'),
        model_num = 0,
    ...
    for cvset in ['test']:
        ...
        model.test_loop(
            ...
            output_file = os.path.join(args.output_dir, 'maxima_performance.csv'),
    ...
    os.remove(os.path.join(args.output_dir, 'maxima_fake_split_tmp.csv'))
