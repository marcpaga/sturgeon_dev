...
class BaseTripletModel(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super(BaseTripletModel, self).__init__(*args, **kwargs)
    ...
    def train_step(self, batch):
        ...
        if len(x.shape) > 2:
            ...
            y = torch.repeat_interleave(y, reps)

        p, emb = self.forward(x) # forward through the network
        loss, losses = self.calculate_loss(y = y, p = p, p_emb = emb)
    ...
    def validation_step(self, batch):
        ...
        with torch.no_grad():
            ...
            p, emb = self.forward(x) # forward through the network
            _, losses = self.calculate_loss(y = y, p = p, p_emb = emb)
    ...
    def calculate_loss(self, y, p, p_emb):
        """
        Args:
            y (): labels
            p (): predicted labels
            p_emb (): embeddings of predictions
        """
        losses = dict()
        if p is not None and 'ce' in self.criterions.keys():
            losses['ce'] = self.ce_loss(y, p)

        if p_emb is not None and 'triplet' in self.criterions.keys():
            losses['triplet'] = self.triplet_loss(y, p_emb)

        loss = 0
        for k in losses.keys():
            if len(self.criterions_weights) > 0:
                loss += losses[k]*self.criterions_weights[k]
            else:
                loss += losses[k]
            losses[k] = losses[k].item()
        losses['global'] = loss.item()
        return loss, losses

    def triplet_loss(self, y, p_emb):

        if self.miner is not None:
            indices_tuple = self.miner(p_emb, y)
        else:
            indices_tuple = None
        return self.criterions['triplet'](p_emb, y, indices_tuple)

    def ce_loss(self, y, p):
        return self.criterions['ce'](p, y)
    ...
    def evaluate(self, batch, predictions):
            ...
            """
            y = batch['y'].cpu().numpy()
            p = predictions.argmax(-1).cpu().numpy()
